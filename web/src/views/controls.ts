  ...
  mount(
    ...
    h('div', { class: 'flex flex-col gap-4' }, [
      ...
      h('div', { class: 'flex flex-wrap items-center gap-2 sm:gap-3' }, [
        ...
        h(
          'button',
          { type: 'button', class: 'btn', disabled: state.running, onClick: handlers.onStep },
          'Step',
        ),
...
function stats(s: Snapshot): HTMLElement {
  ...
  return h(
    ...
    items.map(([label, value]) =>
      h('div', { class: 'stat' }, [h('dt', null, label), h('dd', null, String(value))]),
    ),
  );
}
...
function geometry(state: ControlState, handlers: ControlHandlers): HTMLElement {
  ...
  return h(
    'div',
    { class: 'flex flex-wrap items-end gap-3 pt-3', style: 'border-top:1px solid var(--rule)' },
    [
      field('Floors', floors),
      field('Cabins', cabins),
      h(
